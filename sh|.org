...
curl -s "https://loremipsummultilang.com/" |
  python -c "import re,sys;html=sys.stdin.read();m=re.search(r'textSamples=\\{.*?$1:\"(.*?)\"',html,re.S);print(re.sub(r'\\\\u([0-9a-fA-F]{4})',lambda x: chr(int(x.group(1),16)), m.group(1)).replace('\\\\n','\\n').replace('\\\\\"','\"') if m else '')"
#+end_src
