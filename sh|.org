 ...
 (lambda (row)
   (let ((alias (car row))
     ...
     (if (string= alias "mr")
       ...
       (format "function %s() { if [ $# -gt 0 ]; then %s \"$@\"; else %s %s; fi; }"
               alias cmd cmd default))))
 funcs
 "\n")
#+end_src
...
b() {
  rbw list | fzf --no-preview --prompt="Vault > " |
  xargs -r -d\\n rbw get | tr -d '\n' | wl-copy
}
...
files=$(adb shell find /sdcard/Documents/Obsidian_vault/Vault -maxdepth 1 -name '*.md')
[[ -z "$files" ]] && return 1;
dir=~/org/wiki/Personal/Obsidian_vault/phone
...
J(){
cd "$(fd -E \\.git -i -td '^'"$(sed 's/[][^\\."+*?()|{}$#&~-]/\\\0/' <<< "$1")"'.*$' ~/org/wiki/ | ifne sh -c 'awk "{print length,\$0}"|sort -k1,1n |cut -f1 --complement -d " "|fzf -1')"
}
...
#+begin_src bash :tangle ~/bin/mlorem :shebang "#!/bin/bash"

URL="https://loremipsummultilang.com/"
...
if [[ -z "${langs[$lang]}" ]]; then
  ...
  echo "Run $0 --help to list supported languages."
  exit 1
fi

# EXTRACT TEXT FOR THAT LANGUAGE
exec 3<<PY
import re
import sys
