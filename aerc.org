...
,:PROPERTIES:
:header-args: :tangle ~/.config/aerc/accounts.conf
:END:
#+begin_src conf
#+end_src
...
,:PROPERTIES:
:header-args: :tangle ~/.config/aerc/aerc.conf
:END:
#+begin_src conf
[ui]
index-columns=date<=,name<20,flags>=,subject<*,notmuch>=
# column-notmuch={{ .Labels }}
column-notmuch= \
{{- (map .Labels \
        (exclude .Folder) \
...
* Binds
:PROPERTIES:
:header-args: :tangle ~/.config/aerc/binds.conf
:END:
#+begin_src conf
