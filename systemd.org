...
*** TODO Script
Add new messages notif
#+begin_src sh :tangle ~/.config/notmuch/mail-sync.sh :shebang "#!/bin/sh"
MBSYNC=$(pgrep mbsync)
NOTMUCH=$(pgrep notmuch)

if [ -n "$MBSYNC" -o -n "$NOTMUCH" ]; then
    dunstify -u critical "Already running one instance of mbsync or notmuch. Exiting..."
    exit 0
fi
...
#     dunstify "Email update" "Deleted $emails_to_deleted Email(s)"
# fi
...
ConditionPathIsDirectory=%h/.archive

[Service]
Type=oneshot
ExecCondition=bash -c 'lsof +D %h/tmp | ifne true'
ExecStart=/usr/bin/find %h/tmp -mindepth 1 -maxdepth 1 -exec mv --backup=numbered -t %h/.archive {} +
#+end_src
...
[Unit]
Description=Switch to light theme and wallpaper

[Service]
Type=oneshot
ExecStart=/usr/bin/gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'
ExecStart=/usr/bin/hyprctl hyprpaper wallpaper ",%h/dots/pics/wallpapers/Mountain.jpg"
...
[Unit]
Description=Switch to dark theme and wallpaper

[Service]
Type=oneshot
ExecStart=/usr/bin/gsettings set org.gnome.desktop.interface color-scheme 'prefer-dark'
ExecStart=/usr/bin/hyprctl hyprpaper wallpaper ",%h/dots/pics/wallpapers/kanagawa_vaporwave.jpg"
#+end_src
...
PathChanged=%h/.config/waybar/style-dark.css
PathChanged=%h/.config/waybar/style-light.css
...
#+begin_src conf :tangle ~/.config/systemd/user/ocr.service
[Unit]
Description=Set eng as default language

[Service]
Type=oneshot
ExecStart=/usr/bin/sh -c 'echo eng > $${HOME}/.cache/ocr-lang'
#+end_src

* Gitwatch

#+begin_src conf :tangle ~/.config/systemd/user/gitwatch@.service
[Unit]
...
* Anki
#+begin_src conf :tangle (if (string= (string-trim (org-babel-ref-resolve "chassis")) "desktop") "~/.config/systemd/user/anki.service" "no")
[Unit]
Description=Anki Sync Server
After=network.target

[Service]
EnvironmentFile=%h/.config/anki-sync-server
