...
;; key to swap two keys? e.g. difficult key combination for space
...
- [ ] crea keybind that makes next keypress shifted until another keypress is pressed... for ^W> and the likes
...
;; useful for (emacs lisp)
  ...
  ;; - and = similar to j and k to zoom in pdfs, etc...
   ralt (tap-hold 175 175 - (tap-hold-press-timeout 0 100 - \ (macro S--)))
  ;; ralt (tap-hold-press-timeout 175 175 \ \ (macro S-\))
  ;; ralt esc

  ;;lctl (macro S-3)
   ;; lctl rpt-any ;; ... doesn't work with c-m-s-v though? for emacs
  ;;lctl (macro S-1) ;; near | and vertical aligned with 1/!
   ;; layout picker via left ctrl hold; tap repeats last key (keeps rpt-any)
   ;;lctl (tap-hold-press-timeout 175 175 rpt-any (layer-while-held layout) rpt-any)
   lctl (layer-while-held layout)
  rctl (layer-switch layout)
...
;; how to distinguish between trackpad upper (square brackets) and lower buttons (maybe mouse buttons)?
...
** Macros
create mapping for bindings? like specify source and target like in vim (trigger_bind, lhs, lhs_end, rhs, rhs_end)
...
d4 (tap-hold 175 175 4 (tap-hold-press-timeout 0 230 4 (layer-while-held TV) (macro S-4)))
...
_ _ _ _     _ _ _ (layer-switch base)
_ _ _
)
#+end_src
...
** Layout switcher
Hold to pick temporary layout; tap to reset to default (kanata+kb).

#+begin_src kbd
(defalias
  ;; hyprctl switchxkblayout <device> <index>
  kbd-ru   (cmd hyprctl switchxkblayout kanata 1)
  kbd-ar   (cmd hyprctl switchxkblayout kanata 2)
  kbd-dev  (cmd hyprctl switchxkblayout kanata 3)
  ;; kbd-reset (cmd hyprctl switchxkblayout kanata 0)
  ;; kbd-reset (cmd systemctl --user restart kanata)
  kbd-reset (layer-switch base)
)


(deflayer layout
  _
  _  _  (layer-switch audio)  (layer-switch mouse)  (layer-switch nav)  (layer-switch TV)  _  _  _  _  _  _  _  _
  _  _  _  _  @kbd-ru  _  _  @kbd-reset  _  _  _  _  _
  _  @kbd-ar  _  _  _  _  _  _  _  _  _  _  _  _
...
echo '{ "command": ["get_property", "af"] }' | socat - /tmp/mpvsocket | jq -er '.data[]?.name == "lowpass"' ||\
    { echo '{ "command": ["set_property", "af","lowpass=f=400"] }' | socat - /tmp/mpvsocket ; false ;}  &&\
    echo '{ "command": ["set_property", "af",""] }' | socat - /tmp/mpvsocket 
#+end_src

#+begin_src bash :tangle ~/.config/hypr/scripts/raise :noweb yes :shebang "#!/usr/bin/env bash"
echo '{ "command": ["af-command", "scaletempo2", "multiply", "pitch", "1.059463"] }' | socat - /tmp/mpvsocket 
#+end_src

#+begin_src bash :tangle ~/.config/hypr/scripts/lower :noweb yes :shebang "#!/usr/bin/env bash"
echo '{ "command": ["af-command", "scaletempo2", "multiply", "pitch", "0.943874"] }' | socat - /tmp/mpvsocket 
#+end_src


#+begin_src bash :tangle ~/.config/hypr/scripts/goto :noweb yes :shebang "#!/usr/bin/env bash"
# Ensure display environment is set for GUI apps
offset=$(WAYLAND_DISPLAY=wayland-1 zenity --text="Position (h/m/s?)" --entry --title="Jump to position")

[[ $offset =~ ([0-9]+)h ]] && h=${BASH_REMATCH[1]}
[[ $offset =~ ([0-9]+)m ]] && m=${BASH_REMATCH[1]}
[[ $offset =~ ([0-9]+)s?$ ]] && s=${BASH_REMATCH[1]}

playerctl position $((h*3600 + m*60 + s))
#+end_src
...
if [[ <<chassis()>> == "desktop" ]]; then
  cmd=(flatpak run re.fossplant.songrec)
...
goto (cmd bash -c "~/.config/hypr/scripts/goto")
;; uppercase O for playlist?
loop (cmd bash -c "playerctl loop $( [ $(playerctl loop) = None ] && printf Track || printf None )")
raise (cmd bash -c "~/.config/hypr/scripts/raise")
lower (cmd bash -c "~/.config/hypr/scripts/lower")
...
(deflayer audio
  ...
  _ _ _ @recognize     _ @fx @goto @back          VolumeDown  VolumeUp @forward           _ _ _ _
  _ _ _ _              _ _   _     MediaTrackNext VolumeMute  _        _                  _ _
  _ _ @lower MediaPlayPause _ @raise   _     (layer-switch base)
  _ _ _
)
#+end_src
...
(defalias
  ...
  ccx-i (tap-hold-press-timeout 100 200 (macro C-c C-x i) (macro C-c C-x i) (macro C-c C-x S-i))
...
tv-fifo  (cmd bash -c r#"
[ -p /tmp/tvpipe ] && { dunstify "Error" "FIFO already exists"; exit; }
ip=$(arp-scan --localnet | awk -F$'\t' '$3=="Samsung Electronics Co.,Ltd"{print $1}') &&
mkfifo /tmp/tvpipe &&
tail -f /tmp/tvpipe | websocat "wss://$ip:8002/api/v2/channels/samsung.remote.control?token=12345678" --insecure &
dunstify "Success" "TV connected"
"#)

tv-left  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_LEFT","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)
tv-right  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_RIGHT","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)

tv-chdown  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_CHDOWN","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)
tv-chup  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_CHUP","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)

  ;; Playpause
tv-enter  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_ENTER","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)

  ;; Volume
tv-volup  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_VOLUP","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)
tv-voldown  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_VOLDOWN","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)

tv-mute  (cmd bash -c r#"
echo '{"method":"ms.remote.control","params":{"Cmd":"Click","DataOfCmd":"KEY_MUTE","Option":"false","TypeOfRemote":"SendRemoteKey"}}'>> /tmp/tvpipe
"#)
)
...
(deflayer TV
  _
  _        _ _ _         _ _ _        _        _      _           _         _ _ _
  _ _ _ _         _ _ _        _        _      _ _ @tv-chdown _
  _        _ _ _         _ _ @tv-left @tv-voldown @tv-volup @tv-right   _         _ _ @tv-fifo
  _        _ _ _         _ _ _        @tv-chup        @tv-mute      _           _  _ _
  _        _ _ @tv-enter _ _ _        (layer-switch base)
  _        _ _
)
#+end_src
...
#+begin_src kbd
(a caps)  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype à") 100 first-release ()
(a ')  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype á") 100 first-release ()

(e caps)  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype è") 200 first-release ()
(e ')  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype é") 100 first-release ()

(i caps)  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype ì") 100 first-release ()
(i ')  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype í") 100 first-release ()

(o caps)  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype ò") 100 first-release ()
(o ')  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype ó") 100 first-release ()

(u caps)  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype ù") 100 first-release ()
(u ')  (cmd bash -c "WAYLAND_DISPLAY=wayland-1 wtype ú") 100 first-release ()
)
#+end_src
