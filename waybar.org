...
"custom/email": {
  ...
  "interval": 1,
  "on-click": "if systemctl --user is-active mail-sync.timer >/dev/null; then systemctl --user start mail-sync.service && dunstify -u email 'Mail Sync' 'Syncing completed'; else pizauth show personal 2>&1|awk '{print $NF}'|xargs floorp & systemctl --user start mail-sync.timer; fi"
},
#+end_src
