...
"custom/email": {
  "format": "📬 {}",
  "exec": "systemctl --user --no-pager -o json list-timers mail-sync.timer | jq -re 'if length == 0 then \"inactive\" elif .[].left != null then (.[].left / 1000000 - now | strftime(\"%-Mm %Ss\")) else \"sync\" end'",
  "interval": 1,
  "on-click": "if systemctl --user is-active mail-sync.timer >/dev/null; then systemctl --user start mail-sync.service && dunstify -a email 'Mail Sync' 'Syncing completed'; else pizauth show personal 2>&1|awk '{print $NF}'|xargs floorp & systemctl --user start mail-sync.timer; fi"
},
"custom/mic": {
  "format": "🎤 {}%",
  "exec": "pactl get-source-volume @DEFAULT_SOURCE@ | awk '{print $5}' | tr -d '%'",
  "exec-if": "pactl list sources short | grep $(pactl get-default-source) | grep -q RUNNING",
  "interval": 1,
...
** Dark Theme
:PROPERTIES:
:header-args: :tangle ~/.config/waybar/style-dark.css
:END:
...
#workspaces {
    background: linear-gradient(135deg, #6a4a9e 0%, #5a0a7e 50%, #000000 100%);
...
** Light Theme
:PROPERTIES:
:header-args: :tangle ~/.config/waybar/style-light.css
:END:
...
window#waybar {
    ...
    border: none;
    border-bottom: 3px solid transparent;
    background-image: linear-gradient(rgba(245, 240, 250, 0.95), rgba(245, 240, 250, 0.95)),
                      linear-gradient(90deg, #d35dae 0%, #8b5aa8 50%, #5a3a7e 100%);
    background-origin: border-box;
    background-clip: padding-box, border-box;
}
...
#workspaces {
    background: linear-gradient(135deg, #f5e1ff 0%, #e1c3f0 50%, #d0b3e0 100%);
    margin: 6px 12px 6px 6px;
    border: 2px solid rgba(122, 74, 158, 0.4);
    box-shadow: 0 4px 16px rgba(106, 74, 158, 0.2), inset 0 1px rgba(255, 255, 255, 0.8);
    padding: 2px;
}
...
#workspaces button.focused {
    color: #ffffff;
    background: linear-gradient(135deg, #e06dbe 0%, #d35dae 100%);
    box-shadow: 0 2px 8px rgba(211, 93, 174, 0.4), inset 0 1px rgba(255, 255, 255, 0.3);
}

#workspaces button.active {
    color: #4a2a6e;
    background: rgba(211, 93, 174, 0.2);
    box-shadow: inset 0 0 8px rgba(211, 93, 174, 0.3);
}
...
#clock {
    background: linear-gradient(135deg, #f5e1ff 0%, #e1c3f0 100%);
    border: 2px solid rgba(122, 74, 158, 0.4);
    color: #3a2a5e;
    font-weight: 700;
    margin: 6px 0px 6px 12px;
    padding: 2px 8px;
    box-shadow: 0 3px 10px rgba(106, 74, 158, 0.2), inset 0 1px rgba(255, 255, 255, 0.5);
    letter-spacing: 0.5px;
}
...
#battery {
    border: 2px solid rgba(232, 127, 184, 0.6);
    color: #3a2a5e;
    font-weight: 600;
    box-shadow: 0 4px 12px rgba(211, 93, 174, 0.3), inset 0 1px rgba(255, 255, 255, 0.5);
}
...
#pulseaudio,
#custom-mic {
    background: linear-gradient(135deg, #ffffff 0%, #f5f0fa 50%, #f0e5ff 100%);
    border: 2px solid rgba(200, 180, 220, 0.6);
    color: #3a2a5e;
    box-shadow: 0 2px 8px rgba(150, 130, 180, 0.2), inset 0 1px rgba(255, 255, 255, 0.7);
}
...
#custom-email {
  ...
  box-shadow: 0 4px 10px rgba(255, 200, 200, 0.3);
}
